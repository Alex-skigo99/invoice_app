...
import { InvoiceSchema, validateUpdateRequest } from '../types/validation';
import { validateCreateRequest } from '../types/validation';
...
describe('Invoice request validation', () => {
  ...
  describe('validateCreateRequest', () => {
    it('should validate a draft invoice with status field only', () => {
      const draftInvoice = { status: 'draft' }; // only status is required
      expect(() => validateCreateRequest(draftInvoice)).not.toThrow();
    });

    it('should validate a pending invoice with all required fields', () => {
      const pendingInvoice = { ...validInvoice, status: 'pending' };
      expect(() => validateCreateRequest(pendingInvoice)).not.toThrow();
    });

    it('should throw an error for a pending invoice with missing fields', () => {
      const invalidPendingInvoice = { status: 'pending' };
      expect(() => validateCreateRequest(invalidPendingInvoice)).toThrow(ZodError);
    });

    it('should throw an error for a paid invoice', () => {
      const paidInvoice = { status: 'paid' };
      expect(() => validateCreateRequest(paidInvoice)).toThrow('Cannot create an invoice with status "paid"');
    });
  });
  ...
  describe('validateUpdateRequest', () => {
    it('should validate a draft invoice with status field only', () => {
      const draftInvoice = { status: 'draft' }; // only status is required
      expect(() => validateUpdateRequest(draftInvoice)).not.toThrow();
    });

    it('should validate a pending invoice with all required fields', () => {
      const pendingInvoice = { ...validInvoice, status: 'pending' };
      expect(() => validateUpdateRequest(pendingInvoice)).not.toThrow();
    });

    it('should throw an error for a pending invoice with missing fields', () => {
      const invalidPendingInvoice = { status: 'pending' };
      expect(() => validateUpdateRequest(invalidPendingInvoice)).toThrow(ZodError);
    });

    it('should validate a paid invoice with status field only', () => {
      const paidInvoice = { status: 'paid' }; // only status is required
      expect(() => validateUpdateRequest(paidInvoice)).not.toThrow();
    });

    it('should throw an error for a paid invoice with existing other fields', () => {
      const invalidPaidInvoice = { ...validInvoice, status: 'paid' };
      expect(() => validateUpdateRequest(invalidPaidInvoice)).toThrow(ZodError);
    });
  });
});